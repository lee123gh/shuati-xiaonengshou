...
// v2: 基于 _live_verify3 快照（对应远端 commit 3322df88，fix13 最终版）

function readJson(f) { return JSON.parse(fs.readFileSync(path.join(dir, '_live_verify3', 'banks', f), 'utf8').replace(/^\uFEFF/, '')); }
...
files.forEach(f => {
    const p = path.join(dir, '_live_verify3', 'banks', f);
...
out('');
out('===== 第2部分：判分逻辑仿真 =====');
const html = fs.readFileSync(path.join(dir, '_live_verify3', 'index.html'), 'utf8');
...
fs.writeFileSync(path.join(dir, '_smoke_scan_results_v2.txt'), OUT.join('\n'), 'utf8');
console.log(`完成: 数据问题 ${problems.length} 个, 仿真失败 ${simFail} 个`);
console.log('分类统计: ' + JSON.stringify(cat));
console.log('结果已写入 _smoke_scan_results_v2.txt');
